...
// A player reaching zero cards is not automatically finished while the stock
// still exists: the player refills at the end of the bout. Only after the
// stock, including the face-up trump, is exhausted can zero cards be terminal.
function stockExhausted(s){return getRemainingDeck(s)===0&&s.trumpPickedUp;}
function terminalResult(s){if(!stockExhausted(s))return null;if(s.myHand.length===0&&getOpponentTotalCards(s)===0)return 0;if(s.myHand.length===0)return 1;if(getOpponentTotalCards(s)===0)return-1;return null;}
...
function isExactEndgame(s=state){return stockExhausted(s)&&s.oppUnknownCount===0;}
